...
if USE_LOCAL_LLM:
    ...
    OPENAI_MODEL = str(os.getenv("OPENAI_MODEL", "qwen3.6-flash") or "").strip()

TRANSLATION_MAX_WORKERS = max(int(os.getenv("TRANSLATION_MAX_WORKERS", "4") or 1), 1)
TRANSLATION_CHUNK_MAX_CONTAINERS = max(int(os.getenv("TRANSLATION_CHUNK_MAX_CONTAINERS", "16") or 1), 1)
TRANSLATION_CHUNK_MAX_CHARS = max(int(os.getenv("TRANSLATION_CHUNK_MAX_CHARS", "7000") or 200), 200)

LLM_REQUEST_MAX_RETRIES = max(int(os.getenv("LLM_REQUEST_MAX_RETRIES", "2") or 1), 1)
LLM_REQUEST_RETRY_BASE_DELAY = max(float(os.getenv("LLM_REQUEST_RETRY_BASE_DELAY", "0.2") or 0.1), 0.1)
LLM_REQUEST_TIMEOUT_SECONDS = max(float(os.getenv("LLM_REQUEST_TIMEOUT_SECONDS", "60") or 5), 5)
